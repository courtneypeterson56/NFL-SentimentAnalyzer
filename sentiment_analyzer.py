...
f = pd.read_csv('tweets.nfl.2012.postgame.outputALL.csv')
print(f.shape)
print(f.head())
...
print("Won Cleaned:", cleaned_won[0:3])
...
print("Lost Cleaned:", cleaned_lost[0:3])
...
# print(lost_games[0:1])
# print(cleaned_lost[0:1])
# test = TextBlob(cleaned_lost[1])
...
#wordcloud of word frequencies for all games
wordcloud = WordCloud(width=900,height=500, max_words=1628,relative_scaling=1,normalize_plurals=False,background_color='white').generate_from_frequencies(freqlist_all)
plt.imshow(wordcloud, interpolation='bilinear')
plt.axis("off")
plt.show()
"""
